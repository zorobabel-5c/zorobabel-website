...
<main class="relative max-w-[800px] mx-auto">
	...
	>
		<a href="/" class="z-20" onclick={toggleSideNav}>
			<img
		...
		>
			...
			<li id="search">
				<button class="w-4" onclick={() => (showSearchModal = true)}>
	...
	{#if showSearchModal}
		<section
			...
			onclick={self(() => (showSearchModal = false))}
		>
				...
				{#if !$searchStore.fetching}
					<div class="overflow-y-scroll h-[min(100%,90vh)]">
						<List
							...
							getKey={getTitle}
						>
							{#snippet children({ item: film })}
														<p class="py-2 px-8 hover:text-red-500" onclick={() => (showSearchModal = false)}>
						...
						<List  items={series} shouldScrollIntoView={false} getKey={getTitle}>
							{#snippet children({ item: serie })}
														<p class="py-2 px-8 hover:text-red-500" onclick={() => (showSearchModal = false)}>
