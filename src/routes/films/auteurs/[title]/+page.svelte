	...
	<section class="overflow-y-scroll h-[calc(100vh-120px)]">
		...
		<div class="my-4 text-center">
			...
			{#if currentFilm?.coproduction}
				<span>. In Coproduction with {@html currentFilm.coproduction}</span>
			{/if}
			<span />
		</div>
		...
		{#if currentFilm?.prix_obtenus_par_le_film}
			<div class="mt-6 font-light text-sm text-center px-8">
		...
		<ul class="mb-6 grid gap-2">
			...
			<li><b>Contact : </b> <span>{currentFilm?.contact}</span></li>
			<li>
